    ...
    sections: [{
      ...
      children: [
        ...
        issueTable([
          ...
          {
            id: 'ISS-013',
            title: 'Dashboard crash — RangeError: Invalid time zone in formatInTz()',
            severity: 'High',
            status: 'Resolved',
            fix: 'formatInTz() in client/src/lib/date-utils.ts has signature (date, pattern, tz). A call site in dashboard.tsx passed the arguments as (date, tz, pattern) — tz was used as the format string and pattern was passed as the timezone, causing a RangeError. Fixed by correcting the argument order to formatInTz(dt, \'HH:mm:ss dd/MM\', tz).',
          },
          {
            id: 'ISS-014',
            title: 'Traffic Map — no countries coloured (all map polygons grey)',
            severity: 'High',
            status: 'Resolved',
            fix: 'Sippy CDRs return country names like "Pakistan - Mobile", "Bangladesh - BTCL", "India - NLD" which never matched the exact keys in COUNTRY_NAME_TO_NUMERIC, so numericId was always null and no polygon received a fill colour. Added normaliseCountryName() in server/routes.ts that strips operator suffixes, falls back to longest-prefix match, and aggregates all sub-destination CDRs into one country bucket. Also corrected the "answered" check to use (duration > 0 || result === "0") — the Sippy standard for a connected call.',
          },
          {
            id: 'ISS-015',
            title: 'CK Drill-Down time-window chips had no effect on data fetched',
            severity: 'Medium',
            status: 'Resolved',
