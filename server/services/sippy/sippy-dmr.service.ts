...
export async function generateDMR(
  ...
  // ── GOVERNANCE: Always use full GMT calendar day boundaries ───────────────
  // DMR windows are ALWAYS 00:00:00 – 23:59:59 UTC for the target date.
  // This is non-negotiable — it ensures deterministic alignment with Sippy
  // reporting, carrier invoices, reconciliation, and AI assurance.
  const winStart = dayStart(targetDate);  // setUTCHours(0,0,0,0)
  const winEnd   = dayEnd(targetDate);    // setUTCHours(23,59,59,999)
  ...
  function makeRow(
    ...
    return {
      ...
      // ── UTC window — always full GMT calendar day ───────────────────────
      windowStartGmt:    winStart,
      windowEndGmt:      winEnd,
      timezone:          'UTC',
 ...
 * Status of new rows is 'corrected' to signal human-initiated recalculation.
 */
export async function recalculateDMR(
  config: SippyConfig,
  reportDate: string,
): Promise<DMRGenerateResult> {
  return generateDMR(config, new Date(reportDate + 'T00:00:00Z'), { notes: 'Corrected: recalculated by operator' });
}

/**
 * Lock all DMR rows for a specific date.
 * Locked rows are immutable — required state before invoice generation.
 * Only rows with verificationStatus = 'verified' may be locked.
 */
export async function lockDMRDate(reportDate: string): Promise<{ locked: number; skipped: number; error?: string }> {
  try {
    const rows = await storage.listDMRReports({ reportDate, latestVersionOnly: true });
    const unverified = rows.filter(r => r.verificationStatus !== 'verified' && r.verificationStatus !== 'corrected');
    if (unverified.length > 0) {
      return {
        locked: 0,
        skipped: unverified.length,
        error: `Cannot lock: ${unverified.length} row(s) still have unresolved status. Resolve all drifted/critical rows before locking.`,
      };
    }
    let locked = 0;
    for (const row of rows) {
      await storage.updateDMRStatus(row.id, 'locked');
      locked++;
    }
    return { locked, skipped: 0 };
  } catch (err: any) {
    return { locked: 0, skipped: 0, error: err.message };
  }
}

/**
 * Auto-generate DMR for the previous GMT calendar day.
 * Designed to run at 00:05 UTC daily — after the previous day has fully closed.
 */
export async function autoDailyDMR(config: SippyConfig): Promise<void> {
  const now       = new Date();
  const yesterday = new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), now.getUTCDate() - 1));
  const dateStr   = yesterday.toISOString().slice(0, 10);

  // Skip if already generated today
  const existing = await storage.listDMRReports({ reportDate: dateStr, latestVersionOnly: true });
  if (existing.length > 0) {
    console.log(`[dmr-daily] Already have ${existing.length} row(s) for ${dateStr} — skipping.`);
    return;
  }

  console.log(`[dmr-daily] Auto-generating DMR for previous GMT day: ${dateStr}`);
  const result = await generateDMR(config, yesterday, { notes: 'Auto-generated by daily pipeline (00:05 GMT)' });
  console.log(`[dmr-daily] ${dateStr}: ${result.rowsInserted} rows — matched=${result.matched} drifted=${result.drifted} critical=${result.critical}`);
  if (result.errors.length > 0) {
    console.warn(`[dmr-daily] Errors for ${dateStr}:`, result.errors.join('; '));
  }
}
