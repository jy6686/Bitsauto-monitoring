  ...
  const doc = new Document({
    ...
    sections: [{
      ...
      children: [
        ...
        h1('7.  Recent Platform Data-Flow Changes'),
        spacer(60),
        h2('7.1  Traffic Map Country Normalisation'),
        p('Sippy CDRs include raw destination strings such as "Pakistan - Mobile", "Pakistan - PTCL", and "Pakistan Fixed". These were unresolvable by an exact key lookup against the country numeric map, causing all map polygons to remain uncoloured.', { size: 20 }),
        spacer(60),
        p('Resolution — normaliseCountryName() added to /api/traffic-map route:', { bold: true, color: LIGHT_GY, size: 20 }),
        bullet('Step 1: Strip everything after " - " or " – " (operator suffix)'),
        bullet('Step 2: Exact match against COUNTRY_NAME_TO_NUMERIC (179-entry map)'),
        bullet('Step 3: If no exact match, longest-prefix scan (e.g. "Bangladesh BTCL" → "Bangladesh")'),
        bullet('Step 4: All CDRs sharing the same resolved numericId are aggregated into one country bucket'),
        bullet('Step 5: Answered check corrected — (duration > 0 || result === "0") per Sippy standard'),
        spacer(100),
        h2('7.2  CK Drill-Down Hours Parameter'),
        p('The /api/sippy/ck-drilldown endpoint now reads the ?hours= query parameter (integer 1–24) and passes it as the CDR lookback window. Previously the endpoint ignored the parameter and always queried the default 6h window regardless of the chip selection in the frontend.', { size: 20 }),
        spacer(60),
        ...
        bullet('Tab 3: By Destination — searchable table with country, breakout, margin, vendor rate columns'),
        bullet('Tab 4: P&L Report — portal-scrape date-range report (calls scrapeProfitLossReport on sippy.ts)'),
        spacer(60),
        p('All four tabs share the same period selector and fetch from /api/analytics/margin and /api/analytics/pnl respectively.', { size: 19, italic: true }),
        spacer(200),
        divider(),
