...
// Approx centroids [lng, lat] for major telecom destinations
...
// Normalize GeoJSON country names to our API names (uppercase)
...
function getCountryBorder(count: number): string {
  return count > 0 ? '#fff' : '#D1D9E6';
}
...
function WorldMap({
  ...
  const pulseMarkers = useMemo(() =>
    entities
      ...
      .map(e => ({
        name: e.name,
        coords: COUNTRY_COORDS[e.name.toUpperCase()] ?? null,
        count: e.active,
        cr:    e.connectRate,
      }))
  ...
  return (
    <div
      style={{
        position: 'relative', background: '#F0F4FB',
    ...
    >
      ...
      {!compact && (
        <div style={{
          ...
          padding: '12px 16px 0', position: 'absolute', top: 0, left: 0, right: 0, zIndex: 5,
        }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <Map style={{ width: 14, height: 14, color: '#6B7280' }} />
            <span style={{ fontSize: 12, fontWeight: 700, color: '#374151' }}>Live Traffic Map</span>
          </div>
          <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
            ...
            <div style={{ display: 'flex', gap: 5, alignItems: 'center' }}>
              {[
                { color: '#FCD34D', label: '1–5' },
                { color: '#F59E0B', label: '6–15' },
                { color: '#EF4444', label: '16–40' },
                { color: '#7C3AED', label: '40+' },
              ].map(({ color, label }) => (
      ...
      <ComposableMap
        ...
        projectionConfig={{ scale: compact ? 100 : 120, center: [20, 12] }}
      ...
      >
            ...
            geographies.map(geo => {
              ...
              return (
                <Geography
                  ...
                  stroke={getCountryBorder(count)}
                  strokeWidth={isActive ? 1.2 : 0.35}
                  style={{
                    default: { outline: 'none' },
                    hover: {
                      outline: 'none',
                      fill: isActive ? '#7C3AED' : '#CDD5E0',
                      cursor: isActive ? 'pointer' : 'default',
                    },
                    pressed: { outline: 'none', fill: isActive ? '#6D28D9' : '#CDD5E0' },
                  }}
        ...
        {/* Animated pulse dots on active country centroids */}
          ...
          return (
            <Marker key={name} coordinates={coords}>
              <circle
                ...
                fill={dotColor}
                opacity={0.18}
                style={{ animation: `be2-pulse ${1.8 + (count % 5) * 0.2}s infinite` }}
              />
              <circle
                r={r1}
                fill={dotColor}
              ...
              {!compact && count > 3 && (
                <text
                  textAnchor="middle"
                  y={-(r1 + 5)}
                  style={{ fontSize: 9, fontWeight: 700, fill: '#1F2937', paintOrder: 'stroke', stroke: '#fff', strokeWidth: 2 }}
                >
                  {count}
                </text>
              )}
            ...
            {tooltip.data ? (
              <>
                ...
                <div style={{ color: '#9CA3AF', marginTop: 2 }}>
                  {tooltip.data.connectRate}% connected
                  {onCountryClick && ' · click to drill in'}
                </div>
              </>
            ) : (
  ...
  return (
    <div style={{ background: '#fff', border: '1px solid #E6EAF0', borderRadius: 14, padding: '14px 16px', boxShadow: '0 2px 8px rgba(0,0,0,0.04)' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
        ...
        <button onClick={onSeeAll} style={{ fontSize: 10, color, fontWeight: 600, background: 'none', border: 'none', cursor: 'pointer' }}>
          See all →
        </button>
      </div>
      {rows.length === 0
...
export default function BitsEye2Page() {
  ...
  // Per-entity history for drill-down
    ...
    <div style={{ flex: 1, overflow: 'auto', padding: 20, display: 'flex', flexDirection: 'column', gap: 14 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12 }}>
        ...
        <TopTable
          ...
          rows={destSlice?.entities?.slice(0, 5).map(e => ({ name: e.name, active: e.active })) ?? []}
          color="#D97706"
          onSeeAll={() => { setActiveSection('destinations'); setSelectedEntity(null); }}
        />
      </div>
    </div>
  );
  ...
  const entityPanel = (
    <div style={{ flex: 1, overflow: 'auto', padding: 20, display: 'flex', flexDirection: 'column', gap: 14 }}>
      ...
      {selectedEntity ? (
        ...
        <>
          {/* World Map — shown only for Countries dimension */}
