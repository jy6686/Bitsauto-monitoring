#!/usr/bin/env node
/**
 * verify-commercial.mjs — the release gate for the COMMERCIAL CATALOGUE.
 *
 * Named for its subject, not "all", because scripts/verify-all.mjs already exists and verifies
 * Sippy provisioning — an entirely different subsystem. This file was briefly written over the
 * top of it, which is exactly the collision a specific name prevents.
 *
 * Runs every commercial verifier against DATABASE_URL and exits non-zero if any fails.
 *
 *   node scripts/verify-commercial.mjs
 *
