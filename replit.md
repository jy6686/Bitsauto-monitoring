...
## Vol 2 Features
- **BitsEye Drill-Down Analytics** (`/bitseye`): Hierarchical traffic analytics with left-sidebar tree navigation + right-panel large AreaCharts. Tree: Country → KAM → Destination (3 levels deep). Each level has "All Aggregated" (single summed chart), "All" (entity grid), and individual entity views. API endpoint `GET /api/bitseye/per-entity` supports `category=countries|clients|vendors|kam|destinations`, `countryFilter=X` (restrict CDRs to destination country), `kamFilter=Y` (restrict CDRs to KAM's managed clients). Frontend state machine drives: what data to fetch (countries/kams/dests/clients queries), what to show (EntityPanel with Daily + Weekly AreaCharts, KPI strip, StatsTable). Sidebar expansion state tracks expandedCountry/KAM, open sub-categories. Gradient IDs are entity-name-unique to avoid SVG conflicts in grid. Nav: BarChart3 icon, admin+management+viewer roles.
