...
- createAccount SERVER BUG: This Sippy server returns fault 501 "Fatal error" for all createAccount calls — on_payment_action field required but causes crash for any value. WORKAROUND: use updateAccount on an existing account. Fault string extraction fixed (was using wrong regex for struct format).
- PUSHTOTALK account SETUP COMPLETE (2026-04-08): Renamed i_account=1 (was "TEST") to "PUSHTOTALK" via updateAccount. Routing group=4 (Pakistan First Class), tariff=3 (PAK prefixes $0.05/min), IP auth=10.0.0.1 (i_authentication=1, i_protocol=1). Trunk class auto-detected as "first" (ID=1 starts with '1'). Verified via listAuthRules and getAccountInfo. Script: scripts/create-pushtotalk.mjs
