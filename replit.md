...
- **Troubleshooting Guide** (`server/troubleshoot-generator.ts`): `.docx` internal reference covering ISS-001 through ISS-015. New issues: ISS-013 (formatInTz arg order crash), ISS-014 (Traffic Map no coloured polygons — normaliseCountryName fix), ISS-015 (CK Drill-Down hours param ignored). Routes: `GET /api/download/troubleshooting-guide`, `POST /api/download/regenerate-troubleshoot`.
