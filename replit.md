...
- **User Manual** (`server/manual-generator.ts`): 18-section comprehensive `.docx` user manual covering all platform features, role access matrix, process flow diagrams, keyboard shortcuts, troubleshooting, and glossary. Section 7.6 covers the P&L Report tab. Section 14.5 lists all 7 downloadable documents. Generated on demand via "Update Manual" button in Settings → Documentation Downloads. Routes: `POST /api/download/regenerate-manual`, `GET /api/download/user-manual`. Files saved to `/tmp/` (writable in both dev and production).
- **Troubleshooting Guide** (`server/troubleshoot-generator.ts`): 12-section `.docx` internal reference document covering all resolved issues (ISS-001 through ISS-012), critical issue deep dives (isAuthenticated ReferenceError / Vite hang / OIDC timeout), server startup diagnostic flowchart, Sippy credential swap pattern, auth/role rules, CDR cache architecture, common API error codes, deployment checklist, and preventive maintenance procedures. Auto-generates if missing on first download. Routes: `GET /api/download/troubleshooting-guide`, `POST /api/download/regenerate-troubleshoot`. "Update Troubleshooting Guide" button added to Settings → Documentation Downloads.
...
- **Global Fix Button System** (`client/src/components/fix-button.tsx`): Universal 6-phase diagnostic and repair system visible on every page. Floating wrench button (bottom-right, fixed position) auto-hidden for Viewer role. Three-tab modal: **Diagnose** (10-step flow), **Fix History** (Phase 3), **Auto Rules** (Phase 4). Phase 1+2 — Diagnose tab: Step 1 = detect page context (URL → human name), Step 2–3 = backend diagnostics (Sippy credentials, session status, live listActiveCalls, CDR cache, PostgreSQL, FAS/IRSF engine), Step 4 = classify issues (API_FAILURE / AUTH_ERROR / TIMEOUT / NO_DATA / DATA_MISMATCH / BACKEND_ERROR), Step 5 = one-click auto-fix buttons per issue, Step 7 = rule-based root-cause analysis, Step 8–9 = Admin-only extended actions, Step 10 = final validation + auto-recovery stats. Phase 3 — Fix History: `fix_history` DB table stores every fix attempt (page, issueType, component, fixAction, outcome, triggeredBy, performedBy, createdAt). `GET /api/fix/diagnose` enriches each issue with `pastFix` — the last successful fix for that same issue type (shown as "Previously resolved" badge). `GET /api/fix/history` returns last 50 events. `POST /api/fix/attempt` records every attempt (success/failure). Phase 4 — Auto Recovery: background job runs every 2 minutes inside registerRoutes closure. Rule 1 `sippy_retry`: after 3 consecutive Sippy API failures, auto-retries with 5-minute cooldown and logs to fix_history with `triggeredBy=auto`. Rule 2 `cdr_stale_log`: logs alert when CDR cache is stale >15 minutes. `GET /api/fix/auto-rules` exposes current rule state (enabled, consecutive failures, last event, total auto-fixes). Auto Rules tab shows live rule status, planned roadmap (ASR drop, balance threshold). Status dot on button shows live severity. Backend routes all require admin or management role. Mounted in `layout-shell.tsx` next to CommandBar.
