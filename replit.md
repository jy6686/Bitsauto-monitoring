...
- **BitsEye2 Telemetry Platform (FROZEN — Tier 1 Strategic Asset)**: BitsEye2 is not an analytics page — it is the platform's observability and telemetry layer. It powers AI Ops, RCA, Carrier Intelligence, and the Route Copilot. Architecture is frozen for 12+ months; only UX polish and additional graph types are permitted — no redesign or schema changes. Sub-components: Concurrent Snapshot Engine (`snapshotActiveCalls()`, mutex-guarded, multi-switch), Historical Telemetry Warehouse (`/api/bitseye/entity-history`, LIVE·DAILY·WEEKLY spans), Time-Series Graph Engine (4 chart primitives: ConcurrentChart, EntityIntelligenceChart, CpsChart, NocTelemetryChart), Comparative Telemetry Views (entity drill-down, span/metric switcher), Telemetry Retention Architecture (`concurrent_snapshots`, `mos_hourly`, `rtp_quality_history`, `vendor_stability_snapshots`), Graph-Driven RCA Foundation (`computeQScore()`, `arcDrilldown`, `diversityBadge()`). Navigation: BitsEye domain is its own top-nav entry (id: `telemetry`), placed between Operations and Analytics.

## Product
- **BitsEye Telemetry Platform** *(Tier 1 Strategic Differentiator)*: Concurrent snapshot engine, live+historical telemetry warehouse, time-series graph engine, comparative views (today vs yesterday, vendor A vs B), telemetry retention across multiple DB tables, and graph-driven RCA foundation. Most telecom dashboards show only current state — BitsEye provides live + historical + trend + intelligence.
- **Real-time Monitoring**: Live call data, network performance (Jitter, Latency, Packet Loss, MOS), and telecom KPIs (ASR, ACD, PDD).
- **Advanced Analytics**: Revenue & Margin analysis, Cost Optimization Engine, LCR Analyser, Traffic Forecasting.
