...
- **Internal Team Chat** (`/chat`): Real-time WebSocket-based internal messaging for KAMs and team members. Three default channels auto-created on startup: **#general**, **#noc-team**, **#announcements**. Features: live online presence sidebar (shows who's connected with role badge), message persistence in PostgreSQL (`chat_rooms` + `chat_messages` tables), typing indicators, avatar initials with deterministic colour hash, date dividers, grouped messages (sender avatar only shown on first message in sequence), "Create Channel" dialog for admins. WebSocket server at `/api/chat/ws` uses the `ws` package attached to the existing HTTP server. REST endpoints: `GET /api/chat/rooms` (list rooms), `POST /api/chat/rooms` (create room), `GET /api/chat/rooms/:id/messages` (history up to 200 msgs). Accessible to all roles (admin / management / viewer). "Team Chat" link pinned at top of sidebar navigation. Connection status indicator with auto-reconnect every 3s on drop. Ping/pong keepalive every 25s.
