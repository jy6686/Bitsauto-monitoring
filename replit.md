...
- **Rate Cards** (`/rate-cards`, Feature #6 — LIVE): Bulk rate/route uploader with drag-and-drop CSV/Excel import and live diff preview. Each rate card row now has a single "Import" button that opens a full-screen modal. **Step 1 — Dropzone**: drag & drop any `.csv`/`.xlsx`/`.xls` file OR click to browse; the zone highlights on hover/drag with a colour change and helper pill icons. **Step 2 — Auto diff**: on drop/select the file is parsed client-side, entries are sent to `POST /api/rate-cards/:id/diff`, and the modal transitions to the preview step. **Preview UI**: 4 clickable KPI cards (New / Changed / Removed / Unchanged); clicking a card switches the active tab in the detail table below. **Detail tabs**: Changed (prefix, country, old rate → new rate, % delta with trend icon), Added (all new prefixes with rate/country/breakout), Removed (all deleted prefixes). Up to 500 rows shown per tab with a "Showing first N of M" footer. All-clear banner shown when no changes detected. **Confirm bar**: Cancel button + "Confirm Import" button with a count badge (triggers `POST /api/rate-cards/:id/upload` → full replace in DB). "Replace file" link resets to the dropzone step. The separate "Upload" + "Preview" buttons have been replaced with the single "Import" button. Backend endpoints unchanged: `POST /api/rate-cards/:id/upload` (replace) + `POST /api/rate-cards/:id/diff` (compare). Sidebar entry graduated from planned → live.
- **Routing Manager** (`/routing-manager`): 8-tab routing configuration viewer (tabs: Routing Groups, Destination Sets, Connections, QBR, On-Net Viewer, Policy Simulator + shared cache banner/sync). **Tab 5 — On-Net Routing Viewer** (LIVE, Feature #7): Filters routing groups cache to `on_net === true`. Shows 3-KPI summary bar (On-Net Groups / Total Members / Off-Net Groups), cyan info banner, searchable expandable group rows with cyan accent and RgMembersPanel on expand. Data comes from the same cached `/api/routing-cache/routing-groups` query — no extra backend endpoint needed. **Tab 6 — Routing Policy Simulator** (LIVE, Feature #8): Pure-frontend simulation against cached routing data. Inputs: CLI, CLD, routing group selector (all cached groups listed). On Simulate: fetches `/api/routing-cache/routing-groups/:id/detail` for the selected RG, then runs the simulation client-side. Priority/prefix+preference → highlights winning connection (lowest preference, first active member). LCR → shows amber warning that live rate data is needed, then shows full cascade. Weighted → shows blue note, then shows all candidates. Results panel: policy badge, green "Predicted Winner" card (connection, vendor, DS, host, route count), and a numbered waterfall cascade showing all members in preference order with blocked/active/winner states. **Sidebar**: Route Tester, Translation Tester, Prefix Coverage Checker, On-Net Viewer, Policy Simulator all added to/updated in the Routing section dropdown (planned badges removed — all live).
- **Tools** (`/tools`): 7 tabs (+ new Coverage tab). **Prefix Coverage Checker** (LIVE, Feature #9, `?tab=coverage`): Bulk dialplan tester — paste up to 50 CLDs (one per line, comma or semicolon), select optional account, click Run Coverage Check. Runs each through `POST /api/sippy/test-dialplan` sequentially with a live progress bar. Results table: CLD / Result (Allowed ✓ / Blocked ✗) / Tariff / Route count / Best connection / Estimated cost. Summary KPI cards (Total / Allowed / Blocked / Errors). Export CSV button. All three tools (Route Tester `?tab=route`, Translation Tester `?tab=translation`, Coverage `?tab=coverage`) are linked from the Routing sidebar dropdown.
- **Routing Manager** (`/routing-manager`): routing configuration viewer backed by a 15-min local cache of Sippy routing data (PostgreSQL `routing_groups_cache`, `destination_sets_cache`, `connections_cache`). **Tab 1 — Routing Groups** (LIVE): Expandable rows — click any group to load live members from Sippy via `GET /api/routing-cache/routing-groups/:id/detail`, enriched with cached connection/vendor names, host, destination set name, route count, blocked status. Members rendered in a sortable mini-table by preference. **Tab 2 — Destination Sets** (LIVE): Expandable rows — click any set to load live routes from Sippy via `GET /api/sippy/destination-sets/:id/routes`, rendered with prefix, preference, huntstop indicator, timeout, blocked status, and a "Run LCR →" button that navigates to `/lcr-analyser?prefix=X`. **Tab 3 — Connections** (LIVE, Feature #5): Two-view toggle — **List view** (vendor-grouped connection cards) and **Coverage Map view** (prefix-vs-connection gap matrix). Coverage Map fetches live RG members for all cached routing groups in parallel via `GET /api/coverage/matrix`, cross-references cached connections and destination sets, then renders: 3 gap-summary KPI cards (Unused Connections / Orphan DS / Redundancy Gaps — each clickable to reveal gap detail list), an all-clear banner when healthy, and a scrollable Connection × Destination-Set matrix with ✓ cells showing which routing group creates each link. Footer row shows connection count per DS column. **Tab 4 — QBR Dashboard** (LIVE, Feature #3): Quality-Based Routing dashboard — time window selector (1h/4h/12h/24h/3d/7d), 4 KPI cards (Network ASR, Avg ACD, Avg PDD, Active/Degraded Routes), per-vendor quality table sorted by composite QBR Score (40% ASR + 30% ACD + 30% PDD), colour-coded status badges (excellent/good/degraded/critical), ASR/QBR inline progress bars, degraded route alert banner, mobile card view, footer CDR count. Backend: `GET /api/qbr/metrics?hours=N` — reads in-memory CDR cache, groups by vendor/connection, enriches with `getCachedConnections()`, computes ASR/ACD/PDD and composite QBR score (0–100). **Tabs 5–6** (On-Net/Policy Sim): Placeholder cards. Cache banner shows last sync time, item counts, Sync Now button. Backend new endpoints: `GET /api/routing-cache/routing-groups/:id/detail`, `GET /api/sippy/accounts/:id/info`, `GET /api/qbr/metrics`, `GET /api/coverage/matrix`.
