...
- **#2 LCR Analyser** (`/lcr-analyser`): "Routing & Cost Intelligence" tool. Enter a destination number → longest-prefix match against ALL vendor rate cards → ranked cheapest-first table with colour coding (green/amber/red), rate bar, savings vs best (% and absolute), and optional margin column when a client rate card is selected. Summary cards show best route, worst route, max saving, client rate. API: `POST /api/lcr/analyse` → `storage.lcrAnalyse()`. Nav item (GitBranch icon) under admin+management, listed after Test Call in the sidebar.
