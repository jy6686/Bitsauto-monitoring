...
- **Global Fix Button System** (`client/src/components/fix-button.tsx`): Universal diagnostic and repair system visible on every page. Floating wrench button (bottom-right, fixed position) auto-hidden for Viewer role. Opens a 10-step modal: Step 1 = detect page context (URL → human name), Step 2–3 = backend diagnostics (Sippy credentials, session status, live API call, CDR cache, PostgreSQL, FAS/IRSF engine), Step 4 = classify issues (API_FAILURE / AUTH_ERROR / TIMEOUT / NO_DATA / DATA_MISMATCH / BACKEND_ERROR), Step 5 = one-click auto-fix buttons per issue (Retry API, Warm CDR Cache, Check DB, Refresh Accounts, Refresh Vendors), Step 6 = re-check after fix (query cache invalidation), Step 7 = rule-based diagnostic analysis panel (contextual root-cause explanation per issue type), Step 8–9 = Admin-only extended fix actions panel, Step 10 = final validation summary (CDR size, cache age, timing). Status dot on button shows live severity (green/amber/red). "Clear Frontend Cache" button invalidates all TanStack Query caches. Backend: `GET /api/fix/diagnose` runs 6 sequential checks and returns structured results; `POST /api/fix/attempt` with action enum: `retry_sippy` / `warm_cdr_cache` / `check_db` / `refresh_accounts` / `refresh_vendors`. Both routes require admin or management role. Mounted in `layout-shell.tsx` next to CommandBar — renders once, appears on all pages.
