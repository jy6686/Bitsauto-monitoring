/**
 * Create "Pakistan First Class TALK" routing group with policy="prefix,preference"
 * Then confirm everything and update sippy.ts accordingly.
 */
...
console.log('\n╔══════════════════════════════════════════════════════════╗');
console.log('║   Create RG + Test createAccount Fix                    ║');
console.log('╚══════════════════════════════════════════════════════════╝\n');

// ── 1. Create routing group with correct policy ───────────────────────────────
console.log('── Step 1: Create "Pakistan First Class TALK" routing group ──');
const rgResp = await sippyPost(xC('addRoutingGroup', {
  name:                   'Pakistan First Class TALK',
  policy:                 'prefix,preference',
  description:            'Pakistan routing via TALK vendor - First Class tier',
  disable_onnet_routing:  1,
  disable_onnet_voicemail:1,
}));
console.log('addRoutingGroup status:', rgResp.s, '| fault:', getFault(rgResp.b) ?? 'none');
console.log('body:', rgResp.b.slice(0, 400));
let rgTalkId = null;
if (isOk(rgResp)) {
  rgTalkId = extractId(rgResp.b, 'i_routing_group') ?? extractId(rgResp.b, 'i_id');
  console.log(`  ✓ Routing group created! ID: ${rgTalkId}`);
}

// ── 2. Try createAccount with on_payment_action=0 and welcome_call_ivr=0 ──────
console.log('\n── Step 2: Test createAccount with both fields=0 ──');
const caParams = {
  ...
  welcome_call_ivr:    0,   // 0 = disabled
  on_payment_action:   0,   // 0 = no action
...
const ca0 = await sippyPost(xC('createAccount', caParams));
...
if (isOk(ca0)) {
  ...
  console.log(`  ✓ createAccount WORKS! ID: ${id}`);
  if (id) { const d=await sippyPost(xC('deleteAccount',{i_account:id})); console.log(`  cleanup: ${isOk(d)?'deleted':getFault(d.b)}`); }
} else {
  console.log('  Body:', ca0.b.slice(0, 300));
}

// ── 3. Try on_payment_action=1 ────────────────────────────────────────────────
if (!createAccountWorks) {
  console.log('\n── Step 2b: Try on_payment_action=1 ──');
  const ca1 = await sippyPost(xC('createAccount', { ...caParams, username:'testacct56', authname:'testacct56', on_payment_action:1 }));
  console.log('Status:', ca1.s, '| Fault:', getFault(ca1.b) ?? 'none');
  if (isOk(ca1)) {
    const id=extractId(ca1.b,'i_account');
    createAccountWorks = true;
    console.log(`  ✓ Works with on_payment_action=1! ID: ${id}`);
    if (id) await sippyPost(xC('deleteAccount',{i_account:id}));
  }
}

// ── 4. If RG created, add TALK vendor ────────────────────────────────────────
if (rgTalkId) {
  console.log(`\n── Step 3: Add TALK vendor to routing group ${rgTalkId} ──`);
  const rgm = await sippyPost(xC('addRoutingGroupMember', { i_routing_group: rgTalkId, i_vendor: 3, preference: 1, huntstop: 0 }));
  console.log(`  addRoutingGroupMember: ${isOk(rgm) ? '✓ OK' : getFault(rgm.b)}`);

  // Update Aircel account to use new routing group
  console.log('  Updating Aircel (account #4) routing group to new RG...');
  const ua = await sippyPost(xC('updateAccount', { i_account: 4, i_routing_group: rgTalkId }));
  console.log(`  updateAccount routing_group: ${isOk(ua) ? '✓ OK' : getFault(ua.b)}`);
}

// ── 5. Final state ────────────────────────────────────────────────────────────
console.log('\n── Final State ──');
const rgFinal = await sippyPost(xC('listRoutingGroups', {}));
const rgFinalIds = [...rgFinal.b.matchAll(/<name>i_routing_group<\/name>\s*<value>\s*(?:<[a-z]+>)?(\d+)/gi)].map(m => parseInt(m[1]));
const rgFinalNms = [...rgFinal.b.matchAll(/<name>name<\/name>\s*<value>\s*(?:<[a-z]+>)?([^<]*)/gi)].map(m => m[1]);
console.log('  Routing groups:', JSON.stringify(Object.fromEntries(rgFinalIds.map((id, i) => [id, rgFinalNms[i]]))));

console.log('\n╔══════════════════════════════════════════════════════════╗');
console.log('║                PROVISIONING SUMMARY                     ║');
console.log('╚══════════════════════════════════════════════════════════╝');
console.log(`  Aircel account:                  ID=4 (username=aircel, IP=20.0.0.1)`);
console.log(`  TALK vendor:                     ID=3 (connection: 45.59.163.182)`);
console.log(`  Aircel Pakistan tariff:          ID=4 (rates via web UI only)`);
console.log(`  TALK Pakistan tariff:            ID=5 (rates via web UI only)`);
console.log(`  RG "Pakistan First Class TALK":  ${rgTalkId ? `ID=${rgTalkId} ✓` : 'FAILED'}`);
console.log(`  createAccount API:               ${createAccountWorks ? 'WORKS ✓' : 'BROKEN (use updateAccount workaround)'}`);
console.log(`  Balance alerts:                  ${false ? 'configured' : 'setLowBalance not available on this Sippy'}`);
